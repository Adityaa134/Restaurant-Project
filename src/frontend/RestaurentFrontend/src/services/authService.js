...
export class AuthService {
  ...
  async checkUsernameExists(userName) {
    try {
      const response = await axiosInstance.get(
        `/api/Account/UserNameExist?username=${userName}`,
      );
      return response.data;
    } catch (error) {
  ...
  async ConfirmEmail(uid, token) {
    if (token == null || uid == null) throw Error("Uid or token can't be null");
    try {
      const response = await axiosInstance.post(
        `/api/Account/confirm-email-success?uid=${uid}&token=${token}`,
        {
          uid: uid,
          token: token,
        },
  ...
  async ForgotPasswordEmail(email) {
    ...
    try {
      const response = await axiosInstance.get(
        `/api/Account/forgot-password?email=${encodeURIComponent(email)}`,
      );
      return {
        success: true,
  ...
  async GoogleLogin(credentials) {
    try {
      const response = await axiosInstance.post(
        `/api/ExternalLogin/signin-google?credential=${credentials}`,
      );
      return response.data;
    } catch (error) {
